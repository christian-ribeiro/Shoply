...
public class ProductCategoryMap : IEntityTypeConfiguration<ProductCategory>
{
    ...
    {
        builder.HasOne(x => x.CreationUser).WithMany(x => x.ListCreationUserProductCategory).HasForeignKey(x => x.CreationUserId).HasConstraintName("fkey_categoria_produto_id_usuario_criacao").OnDelete(DeleteBehavior.NoAction);
        builder.HasOne(x => x.ChangeUser).WithMany(x => x.ListChangeUserProductCategory).HasForeignKey(x => x.ChangeUserId).HasConstraintName("fkey_categoria_produto_id_usuario_alteracao").OnDelete(DeleteBehavior.NoAction);

        builder.ToTable("categoria_produto");

        builder.HasKey(x => x.Id).HasName("pk_categoria_produto");
