        ...
        where TProcessType : Enum
{
    ...
    public async Task<BaseResult<TOutput?>> Create(TInputCreate inputCreate)
    {
        ...
        return result.IsSuccess ? BaseResult<TOutput?>.Success(result.Value?.FirstOrDefault()) : BaseResult<TOutput?>.Failure().AddError(result.ListNotification?.FirstOrDefault()!);
    }
